...
重点账号维护在项目根目录的 `douyin_watch_accounts.txt`，每行填写一个公开抖音号、`sec_uid` 或完整创作者主页链接。运行时会先低频检查这些账号的当天作品，再执行常规关键词搜索；接口受限或返回空列表时会改用已登录浏览器加载账号主页。首次精确解析成功后会把稳定账号 ID 缓存在 `browser_data/douyin_watch_accounts.json`。如果普通抖音号无法被平台精确搜索到，请从浏览器复制该账号主页链接替换对应行，脚本不会用相似账号冒充目标账号。

图片轮播会逐张使用本地 RapidOCR/ONNX Runtime 识别中文文字。重点账号和“指定作品补漏”中的普通视频会按时长抽取最多 6 帧并识别画面文字，高清帧会先等比例缩放以控制耗时。关键词匹配范围为“作品描述 + 图片/视频 OCR 文本”，Excel 的“关键信息”会注明命中来源、发现方式和图片/帧序号。首次启用前运行 `uv sync` 安装 OCR 依赖；无需单独安装 Tesseract。

### Windows 一键安装版（Beta）

普通使用者可从本 Fork 的 Releases 页面下载 `DouyinOpinionMonitor-*-win-x64-setup.exe`。安装后双击桌面上的“抖音舆情监测”即可使用，不需要另装 Python、Node.js、uv、Chrome 或 OCR 软件。程序内置独立 Chromium，首次运行会显示非商业学习许可；同意后可填写关键词、日期、监测范围和报表目录。如果已知某条作品被搜索漏掉，可把作品链接或数字 ID 粘贴到“指定作品补漏”；支持带 `modal_id` 的搜索/精选页链接。浏览器出现时按页面提示完成抖音登录，后续会保留登录状态。
